...
def fs_afs_ranking(X_tr: np.ndarray, y_tr: np.ndarray,
    ...
    """
    Train AFSLSTM on full features briefly, then rank features by
    mean gate activation values (descending order).
    ...
    # Compute mean gate activations over training set
    model.eval()
    ...
    print(f"[afs-ranking] mean gate: {mean_g.mean():.3f}, std: {mean_g.std():.3f}")
    return np.argsort(-mean_g)
...
def run_combo1_baseline(Xtr, ytr, Xva, yva, Xte, yte, device, epochs, lr, batch, hidden,
    ...
    # P>=0.95 threshold evaluation (MAIN METRIC)
    thr_p95, _ = find_threshold_for_precision(val_result["_y_true"], val_result["_y_prob"], precision_floor=0.95)
    result = evaluate_with_threshold(test_result["_y_true"], test_result["_y_prob"], thr_p95)
...
def run_combo1_proposed(Xtr, ytr, Xva, yva, Xte, yte, ypat_tr, ypat_va, ypat_te,
    ...
    if has_ap:
        ...
        y_pred_p95 = (test_result["_y_prob"] >= thr_p95).astype(int)
        _log_split_recall("1_AFS_LSTM+AP", test_result["_y_true"], y_pred_p95, ypat_te)
...
def run_combo2_proposed(Xtr, ytr, Xva, yva, Xte, yte, ypat_tr, ypat_va, ypat_te,
    ...
    if has_ap:
        ...
        # >>> PATCH: log split recall
        y_pred_p95 = (test_result["_y_prob"] >= thr_p95).astype(int)
        _log_split_recall("2_XGBFS_LSTM+AP", test_result["_y_true"], y_pred_p95, ypat_te)
...
def run_combo3_proposed(Xtr, ytr, Xva, yva, Xte, yte,
    ...
    if has_ap:
        ...
        result = eval_fixed_threshold(model, dl_te, device=device, has_ap=True)
        
        if "_y_true" in result and "_y_pred" in result:
            _log_split_recall("3_MI_RF_MLP+AP", result["_y_true"], result["_y_pred"], ypat_te)
...
def main():
    ...
    print("\n" + "="*80)
    print(f"NO TRAINING PRECISION FLOOR - TOP-K COMPREHENSIVE SWEEP")
    print(f"     - C1: Gate L1 OFF, NO training prec_floor, topk=25~31, ap_lambda=0.02, afs_rank_epochs={args.afs_rank_epochs}")
    print(f"     - C2: NO training prec_floor, topk=37~40, ap_lambda=0.09")
    print(f"     - C3: NO training prec_floor, topk=39/40/42, ap_lambda=0.07")
    if exp_id:
        print(f"     - Experiment ID: {exp_id}")
    print("     - All combos: NO training precision floor (fair comparison)")
    print("     - All evaluations use P>=0.95 threshold (validation-optimized)")
    ...
    # ============================================================================
    # Combo1: AFS-LSTM (Gate L1 OFF, topk=24 and 28)
    # ============================================================================
    # Clamp AFS ranking epochs to [8, 10]
    afs_rank_epochs = max(8, min(10, args.afs_rank_epochs))
    print(f"\n[Combo1-PreFS] Computing AFS ranking (epochs={afs_rank_epochs}, warmup={WARMUP_EPOCHS}, gate_l1=0.0)...")
    
    # Force gate_l1_lambda to 0.0 for Combo1
    global GATE_L1_LAMBDA  # <<< CRITICAL: Make global so it affects fs_afs_ranking and training
    ...
    # AP-lambda for Combo1: 0.02 (gentler than default 0.03)
    ap_lambda_c1 = 0.02
    
    # Loop over topk = [25, 26, 27, 28, 29, 30, 31] (comprehensive fine-grained search)
    for tk in [25, 26, 27, 28, 29, 30, 31]:
        ...
        print(f"\n{'='*80}")
        print(f"[Combo1] AFS-LSTM @ topk={tk} (gate_l1=0.0, ap_lambda={ap_lambda_c1}, afs_rank_epochs={afs_rank_epochs})")
        print(f"{'='*80}")
    ...
    # ============================================================================
    # Combo2: XGB-FS -> LSTM (Precision Maintenance, topk=37/38/39/40)
    # ============================================================================
    ap_lambda_c2 = 0.09  # XGB combo: 0.09 (between 0.08-0.10)
    
    # Loop over topk = [37, 38, 39, 40] (higher feature count range exploration)
    for tk in [37, 38, 39, 40]:
        # Reset seed for each experiment to ensure independence and reproducibility
        torch.manual_seed(args.seed)
        np.random.seed(args.seed)
        if torch.cuda.is_available():
        ...
        print(f"\n{'='*80}")
        print(f"[Combo2] XGB-FS-LSTM @ topk={tk} (no training prec_floor, eval P>=0.95)")
        print(f"{'='*80}")
        ...
        m2b = run_combo2_proposed(X_tr, y_tr, X_va, y_va, X_te, y_te,
                                  ...
                                  neg_guard_lambda=args.neg_guard_lambda,
                                  neg_gamma=args.neg_gamma,
                                  hard_neg_topq=0.30,  # Strengthened for C2
                                  hard_neg_weight=1.8)  # Strengthened for C2
    ...
    # ============================================================================
    # Combo3: MI+RF -> MLP (Precision floor = 0.995*P_base, topk sweep)
    # ============================================================================
    ap_lambda_c3 = 0.07  # MLP combo
    
    # Loop over topk = [39, 40, 42]
    for tk in [39, 40, 42]:
        # Reset seed for each experiment to ensure independence and reproducibility
        torch.manual_seed(args.seed)
        np.random.seed(args.seed)
        ...
        print(f"\n{'='*80}")
        print(f"[Combo3] MI+RF-MLP @ topk={tk} (no training prec_floor, ap_lambda={ap_lambda_c3})")
        print(f"{'='*80}")
